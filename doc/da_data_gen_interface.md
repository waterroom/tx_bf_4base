...
两片 FPGA 各跑独立 8 元 4 波束（合计 16 元），配置协议通过报文 **`Dest_id`** 字段选片：

- **`Dest_id`** = PACKET_FUNCTION 字高 16 位（`da_data[63:48]`）——指定目标片：片 0 → `0x0001`，片 1 → `0x0002`
- 每片 `da_data_gen` 综合时设置参数 `CHIP_ID`（片 0 = `0x0001`，片 1 = `0x0002`）
- decode 只解析 `Dest_id == CHIP_ID` 的报文，**不匹配则整包丢弃**（不影响本片当前配置）
- 两片寄存器映射完全相同（下表通用），配置内容由各自报文决定
