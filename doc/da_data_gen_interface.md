...
| in     | `rst_bf`                           | 1      | 两片同步复位门（高有效）——外部主控给两片同时拉高（≥8 拍滤波），上升沿产生 1 拍数据路径复位脉冲                    |
...
- **`rst_bf` 同步复位脉冲（可选）**：主控可同时拉高两片 `rst_bf`（≥8 拍滤波防抖）→ 每片滤波输出**上升沿产生 1 拍复位脉冲**（rst_tx），数据路径流水清零 1 拍后恢复——两片各复位一拍即同步恢复输出（用脉冲而非电平，避免数据路径在 rst_bf 拉高期间持续停摆）
