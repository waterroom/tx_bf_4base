...
module tb_da_data_gen;
    ...
    logic arst_dac = 1, arst_cmd = 1;   // 分时钟域异步复位 (高有效, 内部各自同步)
    ...
    // DUT
    da_data_gen u_dut (
        .dac_coreclk(dac_coreclk), .arst_dac(arst_dac), .arst_cmd(arst_cmd),
    ...
    initial begin
        ...
        // 复位
        arst_dac = 1; arst_cmd = 1;   // 复位 (高有效)
        repeat(20) @(posedge dac_coreclk);
        arst_dac = 0; arst_cmd = 0;   // 释放
