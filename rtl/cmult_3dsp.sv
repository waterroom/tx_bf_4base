...
//   Cycle 1: Addition (a+b, c+d) → A_W+1 / B_W+1 位 (17bit, 自然防溢出)
//   Cycle 2: Multiplication: P0/P1 = A_W×B_W (16×16), P2 = 17×17
//            17×17 ≤ 18bit 对齐 → 1 DSP (27×18 模式) → 总 3 DSP
//            (不用饱和截位: 饱和会引入 I/Q 不平衡 → 混频镜像 -21dBc)
...
module cmult_3dsp #(
    ...
    // ======== Cycle 1: Addition (A_W+1 / B_W+1 扩展, 不饱和) ========
    // a_re+a_im 用 17bit (A_W+1) 自然防溢出; 不用饱和截位 —
    // 饱和会引入 I/Q 不平衡 (混频镜像 -21dBc 的根因)。
    logic signed [A_W:0]   a_sum_r;                 // A_W+1 = 17bit
    logic signed [B_W:0]   b_sum_r;                 // B_W+1 = 17bit
    ...
    (* use_dsp = "yes" *) logic signed [P2_W-1:0] p2;        // (A_W+1) × (B_W+1) = 17×17
    logic                  v2;
