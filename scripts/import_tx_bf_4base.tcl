...
if {[llength $xcis] > 0} {
    ...
    # 只处理新导入的 IP（避免误生成 BD 内嵌套 IP 如 design_1_axi_emc_0_0）
    foreach xci $xcis {
        set ipn [file tail [file dirname $xci]]
        set ip_objs [get_ips -quiet -all $ipn]
        if {[llength $ip_objs] > 0} {
            # 器件不匹配(锁定)时 upgrade 到当前 part (fsvg1517 vs ffvg1517)
...
puts "  提示: IP 若器件不匹配已自动 upgrade, 确认无 ERROR 即可综合"
